...
export default function HeaderEditorial({ brandName }) {
    ...
    <div className="p-header-editorial">
      <div className="p-header-container">
        <div className="p-header-brand-wrap">
          <div className="p-header-brand">{brandName || (isBlog ? 'KROMA WRITES' : isPortfolio ? 'KROMA GALLERY' : 'KROMA')}</div>
          <span className="p-header-edition">{isBlog ? 'Vol. 4' : isPortfolio ? 'Ed. 2026' : 'No. 12'}</span>
        </div>
        
        <nav className="p-header-nav">
          {isBlog ? (
            <>
              <a href="#editorial-stories" className="active">Stories</a>
              <a href="#editorial-ideas">Thoughts</a>
              <a href="#editorial-archive">Archive</a>
            </>
          ) : isPortfolio ? (
            <>
              <a href="#editorial-works" className="active">Works</a>
        ...
        {isBlog ? (
          <button 
            ...
            style={{ background: 'none', border: 'none', padding: 0, textDecoration: 'underline', fontStyle: 'italic', cursor: 'pointer', fontFamily: 'var(--font-heading)' }}
          >
