...
export default function CheckoutStepConfirm({ shippingData, paymentData, isSubmitting, onPlaceOrder, onBack }) {
  const { state } = useCart();
  const { items } = state;
        ...
        <div className="p-confirm-items-list">
          {items.map((item, idx) => (
            <div key={`${item.product.id}-${idx}`} className="p-confirm-item-row">
              ...
              {item.variant && <span className="p-confirm-item-variant">({item.variant})</span>}
